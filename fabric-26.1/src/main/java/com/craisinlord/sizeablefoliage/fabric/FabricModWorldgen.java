...
public final class FabricModWorldgen {
    ...
    public static void init() {
        addFeature("big_bush", Biomes.FOREST, Biomes.BIRCH_FOREST, Biomes.DARK_FOREST);
    ...
    private static TagKey<Biome> biomeTag(String path) {
        return TagKey.create(Registries.BIOME, Identifier.fromNamespaceAndPath(Constants.MOD_ID, path));
    }

    @SafeVarargs
    private static void addFeature(String path, ResourceKey<Biome>... biomes) {
        addFeature(path, BiomeSelectors.includeByKey(biomes));
    }
    ...
    private static void addFeature(String path, java.util.function.Predicate<net.fabricmc.fabric.api.biome.v1.BiomeSelectionContext> selector) {
        ResourceKey<PlacedFeature> key = ResourceKey.create(Registries.PLACED_FEATURE, Identifier.fromNamespaceAndPath(Constants.MOD_ID, path));
